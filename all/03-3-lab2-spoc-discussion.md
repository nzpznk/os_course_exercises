...
- 访问数据：cpl<=dpl, rpl<=dpl
- 中断响应：cpl<=dpl(门)，cpl>=dpl(段)
...
- 目的：获得页表虚地址，从而在page fault等需要对页表进行修改的时候可以访问到页表。
- 好处：解决方案简单直接；不足：效率低

1. (easy) Linux和Windows分别采用哪种映射机制（线性映射or自映射）？它们的选择背后有什么原因吗？
   - linux：内核使用线性映射，提高效率
   - windows：自映射
...
1. (midd) 在x86-32中，假设页目录的第R个页表项是自映射的，那么：
   ...
   1. 可以通过哪个虚拟地址访问到页目录？
      - 设R的2进制表示为r，那么由于x86-32使用虚拟页表，所以虚拟地址rrr0（二进制）指向页表起始位置
